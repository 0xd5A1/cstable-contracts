...
contract PaymentFarmingProxy is ERC20, Ownable, ICSTFarmingProxy {
    using SafeMath for uint256;

    ICStablePool public pool;

    uint256 public paymentFee = 3_000_000_000_000_000;

    ICSTMinter public bstMinter;
    ...
    event SetMinter(ICSTMinter _minter);
    event SetDev(address _dev);
    event SetToken(ICSTToken _token);
    event AddCoins(address _coin, uint32 index);
    event RemoveCoins(address _coin);
    event SetPool(ICStablePool _pool);
    event Pay(
        address payToken,
    ...
    function setMinter(ICSTMinter _minter) public onlyOwner {
        require(
    ...
    function setPool(ICStablePool _pool) public onlyOwner {
        require(
